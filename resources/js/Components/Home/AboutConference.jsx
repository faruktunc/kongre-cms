...
const AboutConference = () => {
  ...
  const icons = [<Users />, <Star />, <BookOpen />];
  const gradients = [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center gap-6">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
          {(conferenceInfo.highlights ?? []).map((highlight, index) => (
            ...
            >
              <div className={`flex items-center justify-center w-16 h-16 rounded-full mb-4 ${gradients[index % gradients.length]}`}>
                {React.cloneElement(icons[index % icons.length], { className: "w-8 h-8 text-white" })}
              </div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2 text-center">{highlight.title}</h3>
              <p className="text-gray-700 dark:text-gray-300 text-sm text-center">{highlight.description}</p>
            </div>
          ))}
        </div>
      </div>
